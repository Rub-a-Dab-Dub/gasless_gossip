...
let ParticipantsService = class ParticipantsService {
    ...
    async join(userId, dto) {
        ...
        if (existing) {
            throw new common_1.ConflictException('Pseudonym already taken in this room');
        }
    ...
    async leave(userId, dto) {
        const participant = await this.participantRepo.findOne({
            where: { roomId: dto.roomId, userId },
        });
        if (!participant) {
            throw new common_1.NotFoundException('Participant not found in this room');
        }
