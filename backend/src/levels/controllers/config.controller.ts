...
export class ConfigController {
  constructor(private readonly dynamicXpThresholdsService: DynamicXpThresholdsService) {}
  ...
  async getThresholdForLevel(level: number): Promise<{ level: number; xpRequired: number; badgeUnlocked?: string }> {
  ...
  async deactivateThreshold(
    ...
    return { message: `XP threshold for level ${level} deactivated successfully` };
  }
  ...
  @Get('preview/:totalXp')
  @ApiOperation({ summary: 'Preview what level a given XP amount would result in' })
  ...
  async previewLevel(
    @Param('totalXp', ParseIntPipe) totalXp: number,
  ): Promise<{
    totalXp: number;
    level: number;
    currentXp: number;
