...
export class LevelsService {
  ...
  async createUserLevel(createLevelDto: CreateLevelDto): Promise<LevelResponseDto> {
  ...
  async addXpToUser(userId: string, xpToAdd: number): Promise<LevelResponseDto> {
    const level = await this.levelRepository.findOne({
      where: { userId },
    })
    ...
    level.currentXp = newTotalXp - XpThresholdsConfig.getThresholdForLevel(newLevel)
    ...
    if (newLevel > previousLevel) {
      ...
      // Emit level up event
      const levelUpEvent = new LevelUpEvent(userId, previousLevel, newLevel, newTotalXp, badgesUnlocked)
  ...
  async checkLevelUp(userId: string): Promise<LevelResponseDto> {
    ...
    if (currentLevel > level.level) {
      ...
      level.currentXp = level.totalXp - XpThresholdsConfig.getThresholdForLevel(currentLevel)
      level.xpThreshold = XpThresholdsConfig.getNextLevelThreshold(currentLevel)
      level.isLevelUpPending = true
      ...
      // Emit level up event
      const levelUpEvent = new LevelUpEvent(userId, previousLevel, currentLevel, level.totalXp, badgesUnlocked)
  ...
  private mapToResponseDto(level: Level): LevelResponseDto {
    ...
    const progressPercentage = level.xpThreshold > 0 ? Math.round((level.currentXp / level.xpThreshold) * 100) : 0
